...
namespace StoreKit.Application.Abstractions.Repositories
{
    public interface IRepositoryAsync : ITransientService
    {
        ...
        Task<PaginatedResult<TDto>> GetSearchResultsAsync<T, TDto>(int pageNumber, int pageSize, string[] orderBy = null, string keyword = null, Expression<Func<T, bool>> expression = null, CancellationToken cancellationToken = default)
        where T : BaseEntity
        where TDto : IDto;
